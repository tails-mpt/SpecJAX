#!/usr/bin/env bash
# ============================================================================
# DeepSeek-R1-Distill-Qwen-7B Eagle3 training (pure JAX on TPU)
#
# Target: DeepSeek-R1-Distill-Qwen-7B (Qwen2 architecture, 7.6B dense)
# Hardware: TPU v5e-32 single-host (4 chips, 64 GB HBM, TP=4)
# Dataset: 54K mixed (ShareGPT + UltraChat + PerfectBlend)
# Mode: TTT-7, LR 8e-4, 3 epochs
#
# Usage:
#   bash scripts/run/run_exp_ds_r1_qwen_7b.sh [--dry-run]
# ============================================================================
...
# ── Single-host TPU env (use 4 local chips only) ────────────────────────────
...
# ── Paths ────────────────────────────────────────────────────────────────────
TARGET_MODEL="/path/to/shared-storage/deepseek/models/DeepSeek-R1-Distill-Qwen-7B"
TARGET_MODEL_TYPE="qwen2"
DRAFT_INIT=""            # Random init from scratch
DATA_PATH="$REPO_ROOT/training/data/mixed_54k.jsonl"
OUTPUT_DIR="/path/to/checkpoints/ds-r1-qwen-7b-eagle3"
LOG_DIR="/path/to/workspace/logs"
export WANDB_DIR="${WANDB_DIR:-/tmp/wandb}"

# ── Hyperparameters (v5e-4: 16 GB/chip, conservative for TTT=7) ──────────────
BATCH_SIZE=1
GRAD_ACCUM=16      # effective batch = 1 x 16 = 16 per step
NUM_EPOCHS=3
LEARNING_RATE="8e-4"
MAX_LENGTH=512
WARMUP_RATIO="0.03"
...
LOG_EVERY=10
SAVE_EVERY=200
EXP_NAME="ds-r1-qwen-7b-eagle3"
WANDB_PROJECT="ds-r1-qwen-7b-eagle3-experiments"
...
LOG_FILE="$LOG_DIR/exp_ds_r1_qwen_7b_${TIMESTAMP}.log"

echo "=== Starting DS-R1-Qwen-7B Eagle3 training ==="
echo "Log: $LOG_FILE"
echo ""
