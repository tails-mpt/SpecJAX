#!/usr/bin/env bash
# worker_start_qwen3_8b.sh — run on EVERY worker simultaneously.
# Launch via: bash scripts/run/launch_all_workers_qwen3_8b.sh
#
...
TARGET_MODEL="/path/to/models/Qwen3-8B"
TARGET_MODEL_TYPE="qwen3"
DATA_PATH="$REPO_ROOT/training/data/mixed_54k.jsonl"
OUTPUT_DIR="/path/to/workspace/checkpoints/qwen3-8b-eagle3"
...
LOG_FILE="$LOG_DIR/qwen3_8b_eagle3_w${WORKER_IDX}_${TIMESTAMP}.log"
...
python3 -m specjax.train \
    ...
    --log-every         10                 \
    --save-every        0                  \
    --wandb-project     qwen3-8b-eagle3-experiments \
    --wandb-run-name    qwen3-8b-eagle3    \
    ${MAX_STEPS_ARG}                       \
