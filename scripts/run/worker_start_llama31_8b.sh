#!/usr/bin/env bash
# worker_start_llama31_8b.sh — run on EVERY worker simultaneously.
# Launch via: bash scripts/run/launch_all_workers_llama31_8b.sh
#
# Hardware: TPU v4-32, 4 processes x 8 chips = 32 chips, TP=4 DP=8
# Config: B=4, T=4096, grad_accum=2 → effective 64 seqs/step/DP-rank
set -euo pipefail
...
COORDINATOR_IP="${COORDINATOR_IP:-10.0.0.2}"
...
TARGET_MODEL="/path/to/models/Llama-3.1-8B-Instruct"
TARGET_MODEL_TYPE="llama"
DATA_PATH="$REPO_ROOT/training/data/mixed_54k.jsonl"
OUTPUT_DIR="/path/to/workspace/checkpoints/llama31-8b-eagle3"
...
LOG_FILE="$LOG_DIR/llama31_8b_eagle3_w${WORKER_IDX}_${TIMESTAMP}.log"
...
python3 -m specjax.train \
    ...
    --log-every         10                 \
    --save-every        0                  \
    --wandb-project     llama31-8b-eagle3-experiments \
    --wandb-run-name    llama31-8b-eagle3  \
    ${MAX_STEPS_ARG}                       \
