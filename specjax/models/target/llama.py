"""
Llama target model forward pass in pure JAX — frozen target model.

Covers Llama-3.2 family (1B, 3B):
  Standard GQA + RoPE + SiLU-gated MLP — simplest dense transformer.
  No attention bias, no QK norm, no partial RoPE, no FP8, no MoE.

Llama-3.2-3B-Instruct reference config:
  vocab_size=128256, hidden_size=3072, num_hidden_layers=28,
  num_attention_heads=24, num_key_value_heads=8 (GQA 3:1),
  head_dim=128, intermediate_size=8192,
  rope_theta=500000.0, rms_norm_eps=1e-5

Design:
  - Weights stored as flat dict {str: jnp.ndarray} (from safetensors).
...
def load_params(model_path: str, mesh=None) -> tuple[dict, "LlamaConfig"]:
    """
    Load Llama weights from safetensors shards.
    ...
    # Llama 3.2 ties embed/lm_head — alias if missing
...
# ---------------------------------------------------------------------------
# GQA Attention (standard, no bias, no QK norm, full RoPE)
# ---------------------------------------------------------------------------
...
def gqa_forward(
    ...
    """
    Standard GQA with full RoPE. No bias, no QK norm.
    """
    ...
    # -- Full RoPE (all of head_dim) --
    q = q.transpose(0, 2, 1, 3)                   # [B, n_h, T, d]
    k = k.transpose(0, 2, 1, 3)                   # [B, n_kv, T, d]
    ...
    v = v.transpose(0, 2, 1, 3)                   # [B, n_kv, T, d]
    n_groups = n_h // n_kv
    if n_groups > 1:
...
def llama_forward(
    ...
    """
    Full Llama forward pass (no KV cache — training mode).
    ...
    # For Llama-3.2-3B (28 layers): {1, 13, 24}
