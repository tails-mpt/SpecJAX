...
#   - sglang-jax source (set SGLANG_JAX_ROOT or clone to ~/sglang-jax)
#
# Creates a venv at VENV_DIR (default: /tmp/sglang-jax-venv) with all dependencies.

set -euo pipefail

SGLANG_JAX_ROOT="${SGLANG_JAX_ROOT:-$SGLANG_JAX_ROOT}"
...
if [ ! -d "$SGLANG_JAX_ROOT/python" ]; then
    ...
    echo "Clone it with: git clone https://github.com/sgl-project/sglang-jax $SGLANG_JAX_ROOT"
    exit 1
fi
